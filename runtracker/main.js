...
// Handle environment variables if running locally vs in an injected environment
...
let currentConfig = {
    statsConfig: {
        ...
        showAddresses: true,
        showMutual: true,
        showCrew: false, // Default off now
        showVolDispo: true,
        limit: 5
    }
};
...
window.openEditModal = function(id) {
    ...
    document.getElementById('edit_district').value = call.district || '';
...
window.handleFileUpload = function(input) {
    ...
    reader.onload = async (e) => {
        ...
        try {
            ...
            // Check header to detect version
            const header = rows[0].map(h => h.trim().toUpperCase());
            const hasDistrict = header.includes('DISTRICT');
            const distIndex = hasDistrict ? header.indexOf('DISTRICT') : -1;
            ...
            for (let i = 1; i < rows.length; i++) {
                ...
                try {
                    ...
                    // Default (Old): 0=Inc, 1=Date, 2=Nature, 3=Address, 4=Type, 5=Units, 6=MA, 7=Disp, 8=Notes
                    // New: District inserted based on header detection or fallback logic
                    
                    const getVal = (idx) => (row[idx] ? row[idx].trim().toUpperCase() : '');
                    
                    const incNum = getVal(0);
                    if (!incNum) continue;

                    const reported = row[1]?.trim(); 
                    const nature = getVal(2);
                    
                    let district = '';
                    let address = '';
                    let typeRaw = '';
                    let units = '';
                    let mutualAidCombined = '';
                    let disposition = '';
                    let notes = '';

                    if (hasDistrict && distIndex > -1) {
                        district = getVal(distIndex);
                        // Map others by name if possible, otherwise use fixed relative indices
                        // Assuming standard export structure:
                        // Inc, Date, Nature, District, Address, Type, Units, MA, Disp, Notes
                        address = getVal(4);
                        typeRaw = getVal(5);
                        units = getVal(6);
                        mutualAidCombined = row[7]?.trim();
                        disposition = getVal(8);
                        notes = getVal(9);
                    } else {
                        // Old format
                        address = getVal(3);
                        typeRaw = getVal(4);
                        units = getVal(5);
                        mutualAidCombined = row[6]?.trim();
                        disposition = getVal(7);
                        notes = getVal(8);
                    }
                    ...
                    const seqStr = incNum.replace(yearShort + 'HT', '').replace(yearShort + 'ST', '').replace(yearShort + 'PL', '');
...
function calculateNextIncidentId() {
    ...
    const nextId = `${yearShort}ST${seqPadded}`;
...
window.renderTable = function() {
    ...
    let filtered = allCalls.filter(call => {
        ...
        // District
        if (filterDistrict && call.district !== filterDistrict) return false;
    ...
    filtered.forEach(call => {
        ...
        tr.innerHTML = `
            ...
            <td class="p-4 text-center font-bold text-gray-400">${call.district || '-'}</td>
            ...
            if (givenListEl) {
                ...
                if (sortedGiven.length === 0) {
                    ...
                    sortedGiven.forEach(([name, count]) => {
                        ...
                        li.innerHTML = `<span class="uppercase font-medium truncate pr-2" title="${name}">${name}</span> <span class="bg-gray-700 text-white text-[10px] px-2 py-0.5 rounded-full font-bold">${count}</span>`;
            ...
            if(distListEl) {
                ...
                if (sortedDist.length === 0) {
                    ...
                    sortedDist.forEach(([name, count]) => {
                        ...
                        <div class="w-full bg-gray-700 rounded-full h-2">
                            <div class="bg-indigo-500 h-2 rounded-full transition-all duration-500" style="width: ${barWidth}%"></div>
                        </div>
                        `;
